...
def lifeline(ran, opts, op):
	...
	print("\nLifelines -->", lifelines[0],", ", lifelines[1],
		", ", lifelines[2], ", ", lifelines[3], '\n')
	
	if list_life == []:
		print("You don't have any lifelines remaining\t")
		return None
	
	print("Press (1) for Audience, (2) for 50:50, (3) for double dip or (4) to flip the question\n")
...
def doubleDip(ran):
	...
	if answer[ran] == trial1:
		...
		print("\nYour first trial is wrong! Choose another..\n")
		trial2 = int(input())
		...
		if answer[ran] == trial2:
			...
			print("\nYour second trial is also wrong..Better luck next time!\n")
...
'Which image appears on the flip side of the new 2000 ₹ Note, launched in 2016?',
...
while(wrong != True):
	...
	print("\nWould you like to take a Lifeline? If yes, press [9] or you can quit by pressing [0] \n")
	...
	if give_answer == 9:
		...
		if condition <= 4:
			...
			if great == 0:
				...
				print("\nChoose among any Option or press [0] to quit\n")
				give_ansr = int(input())
		...
		if give_answer == 0:
			...
			print("\nCorrect answer! You have won ₹", end="")
			correct += 1
			total_amt = amount(correct)
...
print("\nYour winning amount is ₹",total_amt)